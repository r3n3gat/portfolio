...
const ProjectModal = ({ data, isOpen, setIsOpen }) => {
  ...
  return (
    ...
    >
      <div
        className={`scrollBar z-10 h-full w-screen overflow-y-auto border-l-[5px] border-solid border-blue-global bg-off-white p-4 shadow-lg transition-transform duration-[800ms] ease-in-out md:w-2/3 lg:w-1/2 ${
        ...
        <div>
          ...
          <div className="mt-6">
            {data.tags.map((tag, index) => (
              <span
                key={index}
                className=" mb-2 mr-3 mt-2 inline-block rounded-xl bg-blue-global px-3 py-1 text-sm text-off-white"
              >
                {tag}{" "}
              </span>
            ))}
          ...
          <div className={`mt-7 flex items-end justify-end`}>
            ...
            {data.github && (
              ...
              >
                <span className="ease absolute left-0 top-0 h-0 w-0 border-t-2 border-blue-light transition-all duration-200 group-hover:w-full"></span>
                <span className="ease absolute bottom-0 right-0 h-0 w-0 border-b-2 border-blue-light transition-all duration-200 group-hover:w-full"></span>
                <span className="ease absolute left-0 top-0 h-0 w-full bg-blue-light transition-all delay-200 duration-300 group-hover:h-full"></span>
                <span className="ease absolute bottom-0 left-0 h-0 w-full bg-blue-light transition-all delay-200 duration-300 group-hover:h-full"></span>
                <span className="absolute inset-0 h-full w-full bg-blue-global opacity-0 delay-300 duration-300 group-hover:opacity-100"></span>
